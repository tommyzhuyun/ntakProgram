...
class Hspice(Spice):
    ...
    def simulate(self, sp_filename, item, cir_file=None, spice_type=None):
        ...
        # cir_file のファイル名が、ML 用の pid を含むパターンだったら、pid を取得
        if EvalCir.ml_mode:
            # 機械学習モード
            # cir_file の絶対パスから pid を取得
            pattern_list = re.findall("/ML/(.*)/Sp_Data/", os.path.abspath(cir_file))
